...
namespace Dolphin { namespace plugins {
    ...
    AssemblyWidget::AssemblyWidget(AssemblyPlugin& _plugin, QWidget* _parent)
      : QWidget(_parent), Ui_AssemblyWidget(),
        plugin_(_plugin) {
    ...
    void AssemblyWidget::on_loadFileButton_pressed()
    {
      ...
      dolphinOut() << "Writing statistics to: " << statFile.fileName().toStdString() << std::endl;
      ...
      {
        dolphinErr() << "This file extension is not supported: " << selectedFileinfo.suffix().toStdString() << std::endl;
      }
    ...
    void AssemblyWidget::on_saveInstructionsButton_pressed()
    {
      ...
      for(int level = 0; level < legoCloudNode->getLegoCloud()->getLevelNumber(); level++)
      {
        ...
        dolphinOut() << "Saving: " << filePathLevel.toStdString().c_str() << std::endl;
    ...
    void AssemblyWidget::on_finalizeButton_pressed()
    {
      ...
      dolphinOut() << "Finalization done, the instructions can be saved." << std::endl;
    }
    ...
    void AssemblyWidget::loadFile(const QString &filePath, int voxelizationResolution)
    {
      ...
      {
        dolphinErr() << "Assembly Plugin > Unable to open file: " << filePath.toStdString().c_str() << std::endl;
        return;
      }
      ...
      if(isMeshExtensionSupported(selectedFileinfo.suffix()))
      {
        ...
        dolphinOut() << "Removing file: " << binvoxFilePath.toStdString() << std::endl;
        binvoxFile.remove();
        ...
        dolphinOut() << QCoreApplication::applicationFilePath().toStdString() << std::endl;
        if(!binvoxProgramFileInfo.exists())
        {
          ...
          if(!binvoxProgramFileInfo.exists())
          {
            ...
            {
              dolphinErr() << "Assembly Plugin: the binvox executable was not found." << std::endl;
              return;
            }
        ...
        {
          dolphinOut() << "Removing file: " << binvoxProgramOutputFile.fileName().toStdString().c_str() << std::endl;
        ...
        if(!binvoxProgramOutputFile.exists())
        {
          dolphinErr() << "The file could not be voxelized with binvox. The format is probably not correctly handled by binvox or the software does not have administrative rights." << std::endl;
          return;
        }
